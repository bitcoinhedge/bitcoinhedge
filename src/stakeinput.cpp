...
// The zBTCHG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBTCHG is a hash of the serial
...
{
    //Create an output returning the zBTCHG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBTCHGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBTCHG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBTCHG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBTCHGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBTCHG output", __func__);
...
{
    CzBTCHGTracker* zbtchgTracker = pwallet->zbtchgTracker.get();
    CMintMeta meta;
    if (!zbtchgTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbtchgTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BTCHG stake is the outpoint
