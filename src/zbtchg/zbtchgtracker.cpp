...
CzBTCHGTracker::CzBTCHGTracker(std::string strWalletFile)
{
...
CzBTCHGTracker::~CzBTCHGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBTCHGTracker::Init()
{
...
bool CzBTCHGTracker::Archive(CMintMeta& meta)
{
...
bool CzBTCHGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBTCHGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBTCHGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBTCHGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBTCHGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBTCHGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBTCHGTracker::GetSerialHashes()
{
...
CAmount CzBTCHGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBTCHGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBTCHGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBTCHGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBTCHGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBTCHGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBTCHGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBTCHGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBTCHGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBTCHGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBTCHGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBTCHGWallet* zBTCHGWallet)
{
    bool iszBTCHGWalletInitialized = (NULL != zBTCHGWallet);
    CMintMeta meta;
    ...
    if (! iszBTCHGWalletInitialized)
        zBTCHGWallet = new CzBTCHGWallet(strWalletFile);
    meta.isSeedCorrect = zBTCHGWallet->CheckSeed(dMint);
    if (! iszBTCHGWalletInitialized)
        delete zBTCHGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBTCHGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBTCHGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBTCHGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBTCHGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBTCHGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBTCHGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBTCHGWallet* zBTCHGWallet = new CzBTCHGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBTCHGWallet);
        }
        delete zBTCHGWallet;
        LogPrint("zero", "%s: added %d dzbtchg from DB\n", __func__, listDeterministicDB.size());
    }
