...
CzBTCHGWallet::CzBTCHGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbtchg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBTCHGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBTCHGSeed_deprecated()) {
                    LogPrintf("%s: Updated zBTCHG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbtchg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbtchg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBTCHGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBTCHGCount(nCountLastUsed);
    else if (!walletdb.ReadZBTCHGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBTCHGWallet::Lock()
{
    seedMaster = 0;
}

void CzBTCHGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBTCHGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBTCHG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBTCHGWallet::LoadMintPoolFromDB()
{
...
void CzBTCHGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBTCHGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBTCHGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbtchgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBTCHGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBTCHG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbtchgTracker which also adds to database
    pwalletMain->zbtchgTracker->Add(dMint, true);
...
void CzBTCHGWallet::SeedToZBTCHG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBTCHGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBTCHGWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZBTCHGCount(nCountLastUsed);
...
void CzBTCHGWallet::GenerateDeterministicZBTCHG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBTCHGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBTCHGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBTCHGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
