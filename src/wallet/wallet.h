...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zbtchgspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BTCHG at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBTCHG send
enum ZerocoinSpendStatus {
    ZBTCHG_SPEND_OKAY = 0,                            // No error
    ZBTCHG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBTCHG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBTCHG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBTCHG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBTCHG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBTCHG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBTCHG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBTCHG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBTCHG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBTCHG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBTCHG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBTCHG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBTCHG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBTCHG_SPENT_USED_ZBTCHG = 14,                      // Coin has already been spend
    ZBTCHG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBTCHG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBTCHGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbtchgAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBTCHGTracker> zbtchgTracker;
    ...
    void setZWallet(CzBTCHGWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbtchgTracker = std::unique_ptr<CzBTCHGTracker>(new CzBTCHGTracker(strWalletFile));
    }

    CzBTCHGWallet* getZWallet() { return zwalletMain; }
    ...
    /** zBTCHG reset */
    boost::signals2::signal<void()> NotifyzBTCHGReset;
