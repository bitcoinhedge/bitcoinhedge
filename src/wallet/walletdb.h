...
public:
    ...
    bool WriteZBTCHGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBTCHGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBTCHGSeed_deprecated(uint256& seed);
    bool EraseZBTCHGSeed();
    bool EraseZBTCHGSeed_deprecated();

    bool WriteZBTCHGCount(const uint32_t& nCount);
    bool ReadZBTCHGCount(uint32_t& nCount);
