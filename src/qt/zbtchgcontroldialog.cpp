...
#include "ui_zbtchgcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbtchgTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbtchgTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBTCHG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBTCHG seed used to mint this zBTCHG is not the same as currently hold in the wallet";
            else
