...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bitcoinhedge address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BTCHG.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bitcoinhedge:URI (bitcoinhedge:XyZ...?message=example)
            {
...
static void NotifyzBTCHGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBTCHGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBTCHGReset.connect(boost::bind(NotifyzBTCHGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBTCHGReset.disconnect(boost::bind(NotifyzBTCHGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbtchgTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
