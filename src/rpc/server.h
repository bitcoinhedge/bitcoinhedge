...
/**
 * BITCOINHEDGE RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZbtchgSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzbtchgseed(const UniValue& params, bool fHelp);
extern UniValue getzbtchgseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzbtchg(const UniValue& params, bool fHelp);
extern UniValue dzbtchgstate(const UniValue& params, bool fHelp);
