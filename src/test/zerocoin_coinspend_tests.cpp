...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzBTCHGWallet *czBTCHGWallet = new CzBTCHGWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czBTCHGWallet->GenerateDeterministicZBTCHG(denom, coin, dMint, true);
        czBTCHGWallet->UpdateCount();
